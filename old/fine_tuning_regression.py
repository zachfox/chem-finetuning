...
def check_smiles_len(smiles): 
    for k,smile in enumerate(smiles):
        if len(smile) > args.max_len:
            smiles[k] = smile[:args.max_len]    
    return smiles
...
def load_data(args):
    ...
    data[args.data_column] = check_smiles_len(data[args.data_column])
    ...
    if args.normalize:
        if args.ntasks == 1:
            ...
            train_y = [(y - mean_y) / std_y for y in train_y]
        elif args.ntasks > 1:
            mean_y = np.mean(np.array(train_y),axis=0)  
            std_y = np.std(np.array(train_y),axis=0)
            for i,std in enumerate(std_y):
                if std < 1e-5:
                    std_y[i] = 1.0
            train_y = [[(task - mean_y[i]) / std_y[i] for i,task in enumerate(y) ] for y in train_y ]
    ...
    print('Total Samples Read: %d' % len(data), file=log_file, flush=True)
...
class MaskMSELoss(nn.Module):
    ...
    def forward(self, output , target):
        ...
        loss = F.l1_loss(output_masked, target_masked)
...
def evaluateModel(model, dataloader, write_predictions='', header=None, normalize=None, log_transform=False, return_predictions=False, split='train'):
    ...
    true_y_flat = np.array([val for i,val in enumerate(true_y_flat) if not (np.isnan(val) or np.isnan(pred_y_flat[i]))])
    pred_y_flat = np.array([pred_y_flat[i] for i,val in enumerate(true_y_flat) if not (np.isnan(val) or np.isnan(pred_y_flat[i]))])
    mse = mean_squared_error(true_y_flat, pred_y_flat)
    mae = mean_absolute_error(true_y_flat, pred_y_flat)
    print('# samples not nan: {0}'.format(len(pred_y_flat)))
    r2 = r2_score(true_y_flat, pred_y_flat)
    spearman = spearmanr(true_y_flat, pred_y_flat)[0]
...
#                f.write(line)
    ...
    if return_predictions:
        return (time.time() - t), total_loss / batch_counter, mse, mae, r2, spearman, true_y_flat, pred_y_flat
...
train_x, train_y, val_x, val_y, test_x, test_y = load_data(args)
...
for i in range(args.epochs):
    ...
    valTime, valLoss, valMSE, valMAE, valR2, valSR = evaluateModel(simpleModel, val_loader, split='val')
    ...
    testTime, testLoss, testMSE, testMAE, testR2, testSR = evaluateModel(simpleModel, test_loader, split='test')
    ...
    logger.log({'epoch': i, 'train time': trainTime, 'train loss': trainLoss,
                'val mae':valMAE, 'val time': valTime, 'val loss': valLoss, 'val rmse': np.sqrt(valMSE),'val R2': valR2, 'val spearman' : valSR,
                'test mae':testMAE, 'test time': testTime, 'test loss': testLoss, 'test rmse': np.sqrt(testMSE),'test R2': testR2, 'test spearman' : testSR}
              )
...
valTime, valLoss, valMSE, valMAE, valR2, valSR, valY, valYPred= evaluateModel(simpleModel, val_loader, write_predictions='val', header=args.score_columns, normalize=normalize, log_transform=args.log_transform, return_predictions=True, split='val')
...
testTime, testLoss, testMSE, testMAE, testR2, testSR, testY, testYPred = evaluateModel(simpleModel, test_loader, write_predictions='test', header=args.score_columns, normalize=normalize, log_transform=args.log_transform, return_predictions=True, split='test')
